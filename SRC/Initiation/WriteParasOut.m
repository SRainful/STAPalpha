...
fprintf(IOUT, '                    X    Y    Z   RX   RY   RZ            X            Y            Z\n');
for i = 1:cdata.NUMNP
    fprintf(IOUT, '%10d      %5d%5d%5d%5d%5d%5d      %13.3f%13.3f%13.3f\n', ...
        i, ID(1,i), ID(2,i), ID(3,i), ID(4,i), ID(5,i), ID(6,i), X(i), Y(i), Z(i));
end
sdata.IDOrigin = 0;                              % Delete old ID array

% Write equation numbers（有改动）
ID = sdata.ID;
fprintf(IOUT, '\n\n EQUATION NUMBERS\n');
fprintf(IOUT, '\n        NODE         DEGREES OF FREEDOM\n');
fprintf(IOUT, '       NUMBER\n');
fprintf(IOUT, '          N                  X         Y         Z        RX        RY        RZ\n');
for N=1:cdata.NUMNP
    fprintf(IOUT, ' %10d         %10d%10d%10d%10d%10d%10d\n', ...
        N, ID(1,N), ID(2,N), ID(3,N), ID(4,N), ID(5,N), ID(6,N));
end

% Write the load vector（有改动）
